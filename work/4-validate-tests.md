...
FAIL if: the primary benchmark test builds its run-record `energy_joules`/`co2_grams` from the tracker's `final_emissions_data` without first calling `energy_tracker.stop()` itself in the test body, or the conftest fixture teardown calls `tracker.stop()` unguarded (it must skip its stop when the test already stopped the tracker), or any read of `final_emissions_data` that can execute before a successful `stop()` uses plain attribute access instead of `getattr(..., None)`. This is a static code check — verify it by reading the test file and `conftest.py`; it must not depend on run artifacts existing.
