...
FAIL if: `ab_compare.py` in baseline mode does not write `integration-test/reports/baseline.json` with the v2 schema — at minimum `schema` equal to `v2` plus `n`, `median_iter_ms_mean`, `cv_iter`, and `mde_pct` (per-iteration metrics). The pre-v2 `mean_ms`/`std_ms`/`cv`/`stable`-only shape is a FAIL.
